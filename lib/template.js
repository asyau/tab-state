...
/** A topic that only restates the tab's title adds noise, not context ("Example Domain — ..."). */
function restatesTitle(topic, title) {
  const t = normalizeText(topic);
  const ti = normalizeText(title);
  return !t || (!!ti && (ti.includes(t) || t.includes(ti)));
}
...
export function templateSummary(r, now = Date.now(), thresholds) {
  ...
  const topic = r.description && !restatesTitle(r.description, r.title) ? truncate(r.description, 100) : '';

  let behavior;
  if (bucket === 'ghost') {
    behavior = ms > 0 ? 'Barely opened (under 2s) and never read.' : 'Opened in the background, never viewed.';
  } else if (bucket === 'glanced') {
    // On a short page that fits on screen the scroll depth reads 100%, and "only saw the top 100%" is nonsense.
    behavior = pct > 0 && pct < 90 ? `Skipped after ${dur}, only saw the top ${pct}%.` : `Skipped after ${dur}.`;
  } else {
    const extras = [];
...
/**
 * A fuller, multi-sentence version of templateSummary for the card detail view's "insight"
 * panel — same underlying data, laid out as separate sentences instead of squeezed into one.
 * Deterministic fallback for when no AI provider is configured or reachable.
 */
export function templateInsight(r, now = Date.now(), thresholds) {
  const sentences = [templateSummary(r, now, thresholds)];
  if (r.selectionSnippet) sentences.push(`The last thing you selected: “${truncate(r.selectionSnippet, 140)}”.`);
  if (r.note) sentences.push(`Your note: “${truncate(r.note, 200)}”.`);
